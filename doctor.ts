...
export async function runDoctor(opts: DoctorOptions = {}): Promise<DoctorReport> {
  ...
  // ── Hook drift: a local copy of something the machine level now owns ─────────
  // The precondition the 2026-08-13 loop audit put on moving the turn-end gate up to ~/.claude. The
  // move leaves a repo wired before it with its own copy, both fire, and two copies of a check is
  // drift. hooks.ts owns the matching and explains why it is manifest-based rather than a diff.
  //
  // Warn, never error, and never a fix: deleting a redundant hook is a decision about one repo's
  // settings file, which is the cognitive tier's business (LOOP §2). Info when there is no manifest,
  // because that is every machine that has not mounted the loop and every CI runner, and a row that
  // failed there would be muted inside a week for being wrong about the common case. The detail names
  // the path either way, so "nothing found" and "nothing looked" never read the same.
  try {
    const h = checkHookDrift({ cwd });
    if (!h.manifestFound) {
      checks.push({
        id: "hook-drift",
        severity: "info",
        ok: true,
        label: "local hooks vs the machine mount",
        detail: `not run: no loop manifest at ${h.manifestPath} (generated by ~/.claude/tools/loop-manifest.ts --write)`,
      });
    } else if (h.leftovers.length === 0) {
      checks.push({
        id: "hook-drift",
        severity: "info",
        ok: true,
        label: "local hooks vs the machine mount",
        detail: `${h.localHookCount} local hook(s), none superseded by a machine-level mount`,
      });
    } else {
      const named = h.leftovers
        .map((l) => `${l.file} [${l.event}] ${l.command} — ${l.mechanismId}${l.movedOn ? ` moved ${l.movedOn}` : ""}`)
        .join("; ");
      checks.push({
        id: "hook-drift",
        severity: "warn",
        ok: false,
        label: "local hooks vs the machine mount",
        detail: `${plural(h.leftovers.length, "local hook")} superseded by a machine-level mount, so ${h.leftovers.length === 1 ? "it fires" : "they fire"} twice: ${named}`,
      });
    }
  } catch (err) {
    checks.push({
      ...
      ok: true,
      label: "local hooks vs the machine mount",
      detail: `hook-drift check skipped: ${err instanceof Error ? err.message : String(err)}`,
    });
  }
